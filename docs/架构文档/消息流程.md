...
export const useChatStore = create<ChatStore>((set, get) => ({
  ...
  sendMessage: async (content: string) => {
    ...
    // 4. 通过 IPC 调用主进程
    try {
      const res = await window.electron.chat.send({
        conversationId: activeConversationId,
        content,
      });
      if (!res.success) {
        // 约定：invoke 统一返回 { success, data, error }
...
```typescript
// electron/preload.ts（详见 docs/架构文档/IPC契约.md）
...
handleIpc(
  ...
  async (event, input: { conversationId: string; content: string; modelId?: string }) => {
    ...
    if (!window) throw new Error("无法获取窗口实例");
    return aiService.sendChatMessage(
      { conversationId: input.conversationId, content: input.content, modelId: input.modelId },
      window,
    );
  },
  "发送消息失败",
...
// token/done/error 由主进程主动推送（不包 IpcResponse）
window.webContents.send(IPC_EVENTS.chat.token, { requestId, token });
...
regenerateMessage: async (messageId: string) => {
  ...
  try {
    const res = await window.electron.chat.send({
      conversationId: activeConversationId,
      content: contextMessages[contextMessages.length - 1]?.content ?? "",
    });
...
// src/stores/chat.ts
stopGeneration: () => {
  void window.electron.chat.cancel(currentRequestId);
  set({ isGenerating: false });
},
```
