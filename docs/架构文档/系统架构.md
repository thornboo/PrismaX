...
**位置**: `electron/ipc/channels.ts` + `electron/ipc/handlers.ts` + `electron/preload.ts` + `src/types/electron.d.ts` + `src/lib/ipc.ts`
...
export const IPC_CHANNELS = {
  chat: { send: "chat:send" },
  db: { getConversations: "db:getConversations" },
} as const;

// 2) invoke/handle 统一返回 IpcResponse<T>
// src/types/electron.d.ts
type IpcResponse<T> =
  | { success: true; data: T; error: null }
  | { success: false; data: null; error: string };

// 3) 渲染进程调用只分支 success，不依赖异常
