...
- 数据根目录（可配置）：`<DataRoot>/`
- 单个知识库目录：`<DataRoot>/knowledge-bases/<kbId>/`（当前实现）
...
<kbDir>/
  ...
  meta.sqlite             # 结构化真源（当前：内置 schema_migrations；后续可统一 drizzle-kit）
  blobs/                  # 原始文件/图片等（当前：按 documentId 命名；后续可升级内容寻址/去重）
    <documentId>.<ext>
  index/
...
当前落地说明（与上文“建议”一致但更 KISS）：

- 知识库 `meta.sqlite` 目前采用内置 `schema_migrations`（按版本号递增的 SQL migration 列表），避免继续手写 `IF NOT EXISTS` 漂移。
- 主库（`prismax.db`）后续建议统一迁移体系（可选 drizzle-kit 或沿用相同 runner），避免两套策略并存。
